...
const SuppliersList = ({suppliers, handleChangeSuppliersList, choosenSuppliers}) => {
            ...
            <div className = "d-flex flex-column"  style ={{height : "300px", overflowY : "scroll"}}>
              {
                suppliersFiltred?.map((elem, index)=>(
                    <div>
                        <SupplierItem supplier = {elem} selectAll = {checkedAllState} handleChecked = {handleChangeSuppliersList} listSuppliers = {choosenSuppliers}/>
                        <Divider/>
                    </div> 
                ))
              }
